...
def show_test():
    db = get_db()
    cur = db.execute('select id, fname, lname from users')
    users = cur.fetchall()
    cur = db.execute('select userID, rfid from rfids')
    rfids = cur.fetchall()
    cur = db.execute('select id, userID, averageDistance, totalReps from workouts')
    workouts = cur.fetchall()
    return render_template('test.html',users=users,rfids=rfids, workouts=workouts)
...
def add_rfid():
    ...
    try:
        ...
        db.execute('insert into rfids (userID, rfid) values (?, ?)',
        [jsonData['userID'], jsonData['rfid']])
        db.commit()
        return '200 OK'
    except:
...
def add_workout():
    keys = ('rfid','averageDistance','totalReps')
    db = get_db()
    if request.method == 'POST':
#        try:
            jsonData = request.form
            print "RECIEVED DATA", jsonData
            userID = db.execute('SELECT userID FROM rfids WHERE rfid=?', (jsonData['rfid'],))
            userID = userID.fetchone()
            if userID == None:
                return '400 BAD RFID'
            db.execute('insert into workouts (userID, averageDistance, totalReps) values (?, ?, ?)',
            [userID['userID'], jsonData['averageDistance'], jsonData['totalReps']])
            db.commit()
            return '200 OK'
#        except:
#            print "Error:", sys.exc_info()[0]
